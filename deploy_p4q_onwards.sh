#!/bin/bash
# deploy_p4q_onwards.sh: P4p already running. Wait for it, then deploy P4q → P4r → P4s+P4u+P4t.
# Run from LOCAL machine where SSH key exists.
set -euo pipefail
...
P4Q_COMMIT="e974e95"  # P4q: VAR=0.3+LAMBDA=0.15, target 75% skip
...
wait_for_run() {
    ...
    while true; do
        if ssh_cmd "grep -q 'val_bpb:' $log 2>/dev/null" 2>/dev/null; then
...
run_eval_gates() {
    ...
    ssh_cmd "test -f /workspace/autoresearch/$ckpt && \
        cd /workspace/autoresearch && git checkout trelis/recursive-gate -- eval_gates.py && \
        WANDB_API_KEY=$WANDB nohup uv run python -u eval_gates.py $ckpt > $log 2>&1 &" 2>/dev/null || echo "  [eval_gates: checkpoint not found: $ckpt]"
}

P4P_BPB="N/A"; P4Q_BPB="N/A"; P4R_BPB="N/A"; P4S_BPB="N/A"; P4U_BPB="N/A"; P4T_BPB="N/A"
P4O_BPB="0.924350"  # already complete
P4N_BPB="0.925392"  # already complete

# Step 1: Wait for P4p (already running)
echo "=== [1] Wait for P4p (VAR=0.2 gate 20-min, already running) ==="
if wait_for_run "/workspace/run_p4p.log" "P4p"; then
    P4P_BPB=$(ssh_cmd "strings /workspace/run_p4p.log | grep '^val_bpb:' | tail -1 | awk '{print \$2}'" 2>/dev/null || echo "N/A")
    P4P_GATE=$(ssh_cmd "strings /workspace/run_p4p.log | grep 'final_gate_mean:\|final_gate_std:' | tail -2" 2>/dev/null || echo "")
    echo "  P4p: val_bpb=$P4P_BPB  $P4P_GATE"
    run_eval_gates "checkpoint_p4p-var0.2-gate.pt" "/workspace/eval_gates_p4p.log"
fi

# Step 2: Deploy P4q
echo ""
echo "=== [2] Deploy P4q (VAR=0.3+LAMBDA=0.15, target 75% skip) ==="
deploy_run "$P4Q_COMMIT" "/workspace/run_p4q.log" "P4q"
...
if wait_for_run "/workspace/run_p4q.log" "P4q"; then
    ...
    P4Q_GATE_MEAN=$(ssh_cmd "strings /workspace/run_p4q.log | grep 'final_gate_mean:' | tail -1 | awk '{print \$2}'" 2>/dev/null || echo "N/A")
    echo "  P4q: val_bpb=$P4Q_BPB  gate_mean=$P4Q_GATE_MEAN"
    run_eval_gates "checkpoint_p4q-var0.3-lambda0.15.pt" "/workspace/eval_gates_p4q.log"
fi

# Step 4: Deploy P4r (80-min)
echo ""
echo "=== [4] Deploy P4r (K=2 gate 80-min) ==="
deploy_run "$P4R_COMMIT" "/workspace/run_p4r.log" "P4r"

# Step 5: Wait for P4r
echo ""
echo "=== [5] Wait for P4r (80-min) ==="
if wait_for_run "/workspace/run_p4r.log" "P4r"; then
    ...
    P4R_GATE_MEAN=$(ssh_cmd "strings /workspace/run_p4r.log | grep 'final_gate_mean:' | tail -1 | awk '{print \$2}'" 2>/dev/null || echo "N/A")
    echo "  P4r: val_bpb=$P4R_BPB  gate_mean=$P4R_GATE_MEAN"
    run_eval_gates "checkpoint_p4r-gate-80min.pt" "/workspace/eval_gates_p4r.log"
fi

# Step 6: Deploy P4s (K=4 gate 20-min)
echo ""
echo "=== [6] Deploy P4s (K=4 gate 20-min) ==="
...
# Step 8: Deploy P4u (K=2 gate_from_postlude0 20-min)
echo ""
echo "=== [8] Deploy P4u (K=2 gate_from_postlude0 20-min) ==="
...
if wait_for_run "/workspace/run_p4u.log" "P4u"; then
    ...
    P4U_GATE_MEAN=$(ssh_cmd "strings /workspace/run_p4u.log | grep 'final_gate_mean:' | tail -1 | awk '{print \$2}'" 2>/dev/null || echo "N/A")
    echo "  P4u: val_bpb=$P4U_BPB  gate_mean=$P4U_GATE_MEAN"
    run_eval_gates "checkpoint_p4u-k2-postlude0-gate.pt" "/workspace/eval_gates_p4u.log"
fi

# Step 10: Deploy P4t (K=4 gate 80-min)
echo ""
echo "=== [10] Deploy P4t (K=4 gate 80-min) ==="
deploy_run "$P4T_COMMIT" "/workspace/run_p4t.log" "P4t"

# Step 11: Wait for P4t
echo ""
echo "=== [11] Wait for P4t (80-min) ==="
if wait_for_run "/workspace/run_p4t.log" "P4t"; then
    ...
    P4T_GATE_MEAN=$(ssh_cmd "strings /workspace/run_p4t.log | grep 'final_gate_mean:' | tail -1 | awk '{print \$2}'" 2>/dev/null || echo "N/A")
    echo "  P4t: val_bpb=$P4T_BPB  gate_mean=$P4T_GATE_MEAN"
...
echo "==========================================="
echo "P4n (K=2 no-gate 80-min):           val_bpb=$P4N_BPB"
echo "P4o (standard GPT 80-min):          val_bpb=$P4O_BPB  [GAP vs P4n: +0.001]"
